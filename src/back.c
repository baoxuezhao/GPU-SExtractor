...
//	QMALLOC(field->sigma, float, nb);		/* sigma map */
	...
	if(field->backw != field->backh)
	{
		printf("Error! currently only square bkgnd mesh (bachw == backh) supported\n");
		exit(-1);
	}
	back_estimate(field->backw,
			prefs.backfthresh,
			&(field->backmean),
			&(field->backsig),
			field->nbackfx,
	...
	/*
	QMALLOC(field->dback, float, nb);
	QMALLOC(field->dsigma, float, nb);
	get_backresult(field->back,
			field->sigma,
			&(field->backmean),
			&(field->backsig),
			field->dback,
	...
	//backmean and sigma is 15933.009766, 180.079819
	//printf("back mean and sigma is %f, %f\n", field->backmean, field->backsig);
...

{
	...
	if(abs(sig)>0.0) {
		...
		if(fabs(bkg->sigma/(sig*bkg->qscale)-1) < 0.0) {
...

{
	...
	fthresh = prefs.backfthresh; //0.00000

	nx = field->nbackx;		//64
	ny = field->nbacky; 	//64
	np = field->nback;      //4096
	npx = field->nbackfx/2; //1
	npy = field->nbackfy/2; //1
	npy *= nx; 				//64
...
used in loadstrip (not executed)
 ***/
