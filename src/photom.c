 ...
 */
void computeaperflux(picstruct *field, picstruct *wfield, objstruct *obj, int i)

{
	float r2, raper, raper2, rintlim, rintlim2, rextlim2, mx, my, dx, dx1, dy,
	dy2, offsetx, offsety, scalex, scaley, scale2, ngamma, locarea;
	double tv, sigtv, area, pix, var, backnoise2, gain;
	int x, y, x2, y2, xmin, xmax, ymin, ymax, sx, sy, w, h, fymin, fymax, pflag,
	corrflag, gainflag;
	long pos;
	PIXTYPE *strip, *stript, *wstrip, *wstript, wthresh = 0.0;
	...
	for (y = ymin; y < ymax; y++) {
		...
		for (x = xmin; x < xmax; x++, stript++, wstript++) {
			...
			if ((r2 = dx * dx + dy * dy) < rextlim2) {
				...
				/*------ common cases */
				if ((pix = *stript) <= -BIG
						|| (wfield && (var = *wstript) >= wthresh)) {
					if (corrflag
							&& (x2 = (int) (2 * mx + 0.49999 - x))
							>= 0&& x2<w
							&& (y2=(int)(2*my+0.49999-y))>=fymin && y2<fymax
							&& (pix=*(strip + (pos = (y2%h)*w + x2)))>-BIG) {if (wfield)
							{
								var = *(wstrip + pos);
								if (var>=wthresh)
									pix = var = 0.0;
	...
	if (i < prefs.mag_apersize)
		obj2->mag_aper[i] =
				tv > 0.0 ? -2.5 * log10(tv) + prefs.mag_zeropoint : 99.0;
...

{
	...
	float bkg, ngamma, mx, my, dx, dy, cx2, cy2, cxy, r2, klim, klim2, kmin,
	kmin2, kmax, kmax2, kstep, kmea, kmea2, dxlim, dylim;
	int area, areab, areaden, areanum, x, y, x2, y2, xmin, xmax, ymin, ymax,
	fymin, fymax, w, h, pflag, corrflag, gainflag, pos;
	PIXTYPE *strip, *stript, *dstrip, *dstript, *wstrip, *wstript, *dwstrip,
	*dwstript, pix, wthresh = 0.0, dwthresh = 0.0;
	...
	for (kmin = kstep; (kmax = kmin * 1.2) < klim; kmin += kstep) {
		...
		for (y = ymin; y < ymax; y++) {
			...
			for (x = xmin; x < xmax; x++, dstript++, dwstript++) {
				dx = x - mx;
				dy = y - my;
				if ((r2 = cx2 * dx * dx + cy2 * dy * dy + cxy * dx * dy)
						<= kmax2) {
					if ((pix = *dstript) > -BIG
							&& (!dwfield || (dwfield && *dwstript < dwthresh))) {
						area++;
						if (r2 >= kmin2) {
							munum += pix;
							areanum++;
	...
	if (area) {
		...
		if (obj2->petrofactor * sqrt(obj->a * obj->b)
		> prefs.autoaper[1] / 2.0) {
			cx2 = obj->cxx;
			cy2 = obj->cyy;
			cxy = obj->cxy;
			dxlim = cx2 - cxy * cxy / (4.0 * cy2);
	...
	if (FLAG(obj2.magerr_petro) )
		obj2->magerr_petro =
				obj2->flux_petro > 0.0 ?
						1.086 * obj2->fluxerr_petro / obj2->flux_petro : 99.0;
	if (tv <= 0.0)
		obj2->petrofactor = 0.0;
...

{
	...
	*dwstript, pix, wthresh = 0.0, dwthresh = 0.0;
	...
	//dxlim, dylim, klim2 is 8.558679, 6.557583, 36.000000, 189, 208, 0, 8
	if(abs(obj2->sposx - 198.951) < 0.0001 && abs(obj2->sposy - 1.892) < 0.0001)
		printf("dxlim, dylim, klim2 is %f, %f, %f, %d, %d, %d, %d \n",
				dxlim, dylim, klim2, xmin, xmax, ymin, ymax);
...
//	if(abs(obj2->sposx - 198.951) < 0.0001 && abs(obj2->sposy - 1.892) < 0.0001)
//	{
//		printf("r1, v1, area, areab is %f, %f, %d, %d \n", r1, v1, area, areab);
//	}
	...
	if (area) {
		...
		/*-- Clip boundaries in x and y (bis) */
		/*-- We first check that the derived ellipse is large enough... */
		if (obj2->kronfactor * sqrt(obj->a * obj->b)
		> prefs.autoaper[1] / 2.0) {
			cx2 = obj->cxx;
			cy2 = obj->cyy;
			cxy = obj->cxy;
	...
	if(abs(obj2->sposx - 198.951) < 0.0001 && abs(obj2->sposy - 1.892) < 0.0001)
	{
		printf("flux_auto %f, %d, %d, %d, %d, %d \n", tv, ymin, ymax, xmin, xmax, obj->fdnpix);
	}
	...
	if (FLAG(obj2.magerr_auto) )
		obj2->magerr_auto =
				obj2->flux_auto > 0.0 ?
						1.086 * obj2->fluxerr_auto / obj2->flux_auto : 99.0;
	if (tv <= 0.0)
		obj2->kronfactor = 0.0;
...

{
	...
	if (ati > 1.0)
		...
		ati = 0.0;
	obj2->flux_isocor = obj->flux
			/ (1.0 - 0.196099 * ati - 0.751208 * ati * ati);
...
//		obj2->magerr_best =
//				obj2->flux_best > 0.0 ?
//						1.086 * obj2->fluxerr_best / obj2->flux_best : 99.0;
//
//	/* Mag. SOM-fit */
//	if (FLAG(obj2.mag_somfit) )
//		obj2->mag_somfit =
//				obj2->flux_somfit > 0.0 ?
//						-2.5 * log10(obj2->flux_somfit) + prefs.mag_zeropoint :
//						99.0;
//	if (FLAG(obj2.magerr_somfit) )
//		obj2->magerr_somfit =
//				obj2->flux_somfit > 0.0 ?
//						1.086 * obj2->fluxerr_somfit / obj2->flux_somfit : 99.0;
//
//	/* Mag. models */
...
//		obj2->prof_spheroid_mag =
//				obj2->prof_spheroid_flux > 0.0 ?
//						-2.5 * log10(obj2->prof_spheroid_flux)
//	+ prefs.mag_zeropoint :
//	99.0;
//	if (FLAG(obj2.prof_spheroid_magerr) )
//		obj2->prof_spheroid_magerr =
//				obj2->prof_spheroid_flux > 0.0 ?
...
//		obj2->prof_disk_mag =
//				obj2->prof_disk_flux > 0.0 ?
//						-2.5 * log10(obj2->prof_disk_flux)
//	+ prefs.mag_zeropoint :
//	99.0;
//	if (FLAG(obj2.prof_disk_magerr) )
//		obj2->prof_disk_magerr =
//				obj2->prof_disk_flux > 0.0 ?
//						1.086 * obj2->prof_disk_fluxerr / obj2->prof_disk_flux :
//						99.0;
//
//	if (FLAG(obj2.prof_bar_mag) )
//		obj2->prof_bar_mag =
//				obj2->prof_bar_flux > 0.0 ?
//						-2.5 * log10(obj2->prof_bar_flux)
//	+ prefs.mag_zeropoint :
//	99.0;
//	if (FLAG(obj2.prof_bar_magerr) )
//		obj2->prof_bar_magerr =
//				obj2->prof_bar_flux > 0.0 ?
//						1.086 * obj2->prof_bar_fluxerr / obj2->prof_bar_flux :
//						99.0;
//
//	if (FLAG(obj2.prof_arms_mag) )
//		obj2->prof_arms_mag =
//				obj2->prof_arms_flux > 0.0 ?
//						-2.5 * log10(obj2->prof_arms_flux)
//	+ prefs.mag_zeropoint :
//	99.0;
//	if (FLAG(obj2.prof_arms_magerr) )
//		obj2->prof_arms_magerr =
//				obj2->prof_arms_flux > 0.0 ?
//						1.086 * obj2->prof_arms_fluxerr / obj2->prof_arms_flux :
//						99.0;
//
//	/* Mag. WINdowed */
...
//		obj2->magerr_win =
//				obj2->flux_win > 0.0 ?
//						1.086 * obj2->fluxerr_win / obj2->flux_win : 99.0;
//	/* Mag. GALFIT */
//	if (FLAG(obj2.mag_galfit) )
//		obj2->mag_galfit =
//				obj2->flux_galfit > 0.0 ?
//						-2.5 * log10(obj2->flux_galfit) + prefs.mag_zeropoint :
//						99.0;
//	if (FLAG(obj2.magerr_galfit) )
//		obj2->magerr_galfit =
//				obj2->flux_galfit > 0.0 ?
//						1.086 * obj2->fluxerr_galfit / obj2->flux_galfit : 99.0;
//
//	/* SB units */
//	if (FLAG(obj2.maxmu) )
//		outobj2.maxmu =
